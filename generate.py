...
for i in range(0, ITERATIONS):
    ...
    # Check other output files work out next in sequence for output file name
    output_images = [
    ...
    max_suffix = max([int(f.split('-')[-1]) for f in output_images]) + 1 if len(output_images) > 0 else 1
    output_filename = os.path.join(OUTPUT_DIR, f'{OUTPUT_PREFIX}-{max_suffix}.png')
